...
export default async function DashboardPage() {
  ...
  // --- CALCULAR MÉTRICAS E TABELA PARA "COBERTURA DE AQUISIÇÃO POR MATERIAL" ---
  ...
  if (isPostgres) {
    ...
    // Simula atas vigentes encontradas
    ...
    tableRows = state.needs.map((need) => {
      ...
      const relatedAtas = mapping
        ? state.acquisitionInstruments.filter(
            (i) => i.sourceSystem === "COMPRAS_GOV" && i.itemCode === mapping.externalItemCode
          )
        : [];
    ...
    <AppShell>
      ...
      <section className="mt-6 bg-zinc-50 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-800 rounded-lg p-5">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mt-2">
          ...
          <Link
            href={`/necessidades/${mainNeed?.id}/buscar-cobertura`}
            className="bg-indigo-600 hover:bg-indigo-700 text-white font-medium text-xs px-4 py-2 rounded text-center block font-semibold transition-all hover:shadow shadow-sm"
          >
      ...
      {/* --- MÓDULO ANÁLISE DE COBERTURA DE AQUISIÇÃO POR MATERIAL --- */}
      <section className="mt-6">
        <Card>
          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-zinc-900">Cobertura de aquisição por material</h2>
              <p className="mt-1 text-sm text-zinc-600">
                Acompanhamento analítico da correlação CATMAT de necessidades com atas vigentes e consulta de saldos no Compras.gov.br.
              </p>
            </div>
              ...
              <tbody className="divide-y divide-zinc-100">
                {tableRows.map((row) => (
                  <tr key={row.needId} className="hover:bg-zinc-50 dark:bg-zinc-800/50/50">
                    ...
                    <td>
                      <span className={`px-2 py-0.5 rounded text-xs font-semibold ${row.catmatCode === "Pendente" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"}`}>
                        {row.catmatCode}
                      </span>
                    </td>
