...
export async function POST(request: Request) {
  ...
  routeLog("ARP_ROUTE_RECEIVED", { requestId, needId, analysisId, catmatCode, stage: "ARP_ROUTE_RECEIVED", durationMs: 0, count: 0, status: "RECEIVED" });

  const actor = await getRouteActor();
  if (!actor) return failure(401, requestId, "UNAUTHORIZED", "Autenticacao obrigatoria.", false, { requestId, needId, analysisId, catmatCode, durationMs: Date.now() - routeStartedAt });
  if (!needId) return failure(400, requestId, "MISSING_NEED_ID", "needId obrigatorio.", false, { requestId, analysisId, catmatCode, durationMs: Date.now() - routeStartedAt });
  if (!catmatCode) return failure(400, requestId, "MISSING_CATMAT_CODE", "catmatCode obrigatorio.", false, { requestId, needId, analysisId, durationMs: Date.now() - routeStartedAt });
  if (!catalogMappingId) return failure(400, requestId, "MISSING_CATALOG_MAPPING_ID", "catalogMappingId obrigatorio.", false, { requestId, needId, analysisId, catmatCode, durationMs: Date.now() - routeStartedAt });

  const mode = persistenceMode();
  if (mode !== "postgresql" && process.env.MCL_ALLOW_MEMORY_FALLBACK !== "true" && !demoMemoryFallbackAllowed() && process.env.NODE_ENV !== "test") {
    return failure(503, requestId, "MEMORY_FALLBACK_DISABLED", "DATABASE_URL nao configurada e MCL_ALLOW_MEMORY_FALLBACK=true nao foi definido.", false, { requestId, needId, analysisId, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });
  }

  try {
    const state = getDemoState();
    let mapping = mode === "postgresql" ? await activeCatalogMappingForNeed(state, needId) : activeCatalogMappingForNeedSync(state, needId);
    if (!mapping && body.mappingSnapshot) mapping = body.mappingSnapshot;

    if (!mapping) return failure(400, requestId, "NO_CATMAT_MAPPING", "Confirme um CATMAT antes de consultar atas.", false, { requestId, needId, analysisId, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });
    if (mapping.id !== catalogMappingId) return failure(400, requestId, "INVALID_CATALOG_MAPPING_ID", "O mapeamento informado nao e o CATMAT ativo da necessidade.", false, { requestId, needId, analysisId, catalogMappingId, activeCatalogMappingId: mapping.id, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });
    if (mapping.needId !== needId) return failure(400, requestId, "INVALID_MAPPING_NEED", "O mapeamento nao pertence ao material da necessidade.", false, { requestId, needId, analysisId, catalogMappingId, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });
    if (mapping.externalItemCode !== catmatCode) return failure(400, requestId, "INVALID_CATMAT_CODE", "O CATMAT informado nao corresponde ao mapeamento confirmado.", false, { requestId, needId, analysisId, catalogMappingId, activeCatmatCode: mapping.externalItemCode, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });
    if (mapping.status !== "ACTIVE" && (mapping.status as string) !== "CONFIRMED") return failure(400, requestId, "INVALID_MAPPING_STATUS", "O status do mapeamento deve ser ACTIVE ou CONFIRMED.", false, { requestId, needId, analysisId, catalogMappingId, catmatCode, persistenceMode: mode, durationMs: Date.now() - routeStartedAt });

    routeLog("ARP_MAPPING_VALIDATED", { requestId, needId, analysisId, catmatCode, stage: "ARP_MAPPING_VALIDATED", durationMs: Date.now() - routeStartedAt, count: 1, status: mapping.status });
    ...
    const trace = {
      ...
      dateStart: body.dateStart ?? body.dataVigenciaInicialMin,
      dateEnd: body.dateEnd ?? body.dataVigenciaInicialMax,
      endpoint: result.query.endpoint ?? "/api/ata-registro-preco/1_consultarAta",
