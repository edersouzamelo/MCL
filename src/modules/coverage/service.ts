...
function candidateFromCatalogItem(
  ...
  return {
    ...
    groupCode: item.codigoGrupo !== undefined && item.codigoGrupo !== null ? String(item.codigoGrupo) : undefined,
    classCode: item.codigoClasse !== undefined && item.codigoClasse !== null ? String(item.codigoClasse) : undefined,
    pdmCode: item.codigoPdm !== undefined && item.codigoPdm !== null ? String(item.codigoPdm) : undefined,
  ...
  if (persistenceMode() === "postgresql") {
    ...
    try {
      ...
      if (entries.length === 0 && process.env.MCL_ALLOW_SIMULATED_ARP_FALLBACK === "true") {
        // Fallback de atas simuladas removido a pedido.
      }
