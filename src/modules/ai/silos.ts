  ...
  return {
    ...
    data: {
      silos: [
        ...
        { id: "CREDITOS", label: "Créditos/Tesouro Gerencial", status: "BLOCKED", nature: "Não confiável na main atual", reason: "O store atual contém dados fixos e derivados artificiais, e o schema não oferece escopo organizacional seguro." },
        { id: "GRUPAMENTO", label: "SAG/Centro de Comando", status: "BLOCKED", nature: "Somente navegador", reason: "O snapshot validado fica em localStorage e não está disponível para a rota server-side do Assistente." },
      ],
    },
  };
}
...
export async function queryMclData(
  ...
  if (input.silo === "CREDITOS") {
    return blocked(
      "Consulta financeira bloqueada: a main atual ainda contém valores fixos/derivados e não oferece escopo organizacional seguro. Nenhum número financeiro foi retornado.",
      { records: [] },
    );
  }
  if (input.silo === "GRUPAMENTO") {
    return blocked(
      "Consulta SAG bloqueada: o snapshot validado existe somente no localStorage do navegador e não está acessível ao servidor. Nenhum número do Grupamento foi retornado.",
      { records: [] },
    );
  }
