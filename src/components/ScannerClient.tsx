...
export function ScannerClient() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="rounded border border-zinc-200 dark:border-zinc-800 bg-zinc-950 p-3">
        <video ref={videoRef} className="aspect-video w-full rounded bg-zinc-900 object-cover" muted playsInline />
      </div>
      <div className="space-y-3 rounded border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button type="button" onClick={stopCamera} className="inline-flex items-center gap-2 rounded border border-zinc-300 dark:border-zinc-700 px-3 py-2 font-semibold text-zinc-800 hover:bg-zinc-100">
        ...
        <label className="block text-sm font-medium text-zinc-800">
          Entrada manual
          <input value={manualCode} onChange={(event) => setManualCode(event.target.value)} className="mt-1 w-full rounded border border-zinc-300 dark:border-zinc-700 px-3 py-2" />
        </label>
