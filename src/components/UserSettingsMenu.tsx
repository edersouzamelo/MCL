...
export function UserSettingsMenu() {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {/* Dropdown Menu */}
      <div 
        className={`absolute right-0 mt-3 w-80 rounded-xl bg-zinc-900 shadow-2xl border border-zinc-800 text-zinc-300 z-50 overflow-hidden transform origin-top-right transition-all duration-200 ${
          isOpen 
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button 
                ...
                className={`px-3 py-2.5 text-xs font-semibold rounded-md border transition-all ${theme === "light" ? "bg-zinc-800 border-emerald-600/50 text-emerald-400" : "border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800/50"}`}
              >
                {t.light}
              </button>
              <button 
                onClick={() => setTheme("dark")}
                className={`px-3 py-2.5 text-xs font-semibold rounded-md border transition-all ${theme === "dark" ? "bg-zinc-800 border-emerald-600/50 text-emerald-400" : "border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800/50"}`}
              >
                {t.dark}
              </button>
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button 
                ...
                className={`px-3 py-2.5 text-xs font-semibold rounded-md border transition-all ${!animationsEnabled ? "bg-zinc-800 border-emerald-600/50 text-emerald-400" : "border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800/50"}`}
              >
                OFF
              </button>
              <button 
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["pequena", "media", "grande"] as FontSize[]).map((size) => (
                <button 
                  ...
                  className={`px-2 py-2.5 text-[11px] font-semibold rounded-md border transition-all ${fontSize === size ? "bg-zinc-800 border-emerald-600/50 text-emerald-400" : "border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-800/50"}`}
                >
